...
unbolt_cupola_sub
	...
	lda #89 ; WITH SOME DIFFICULTY, YOU MANAGE TO REMOVE THE BOLTS. AS YOU PUSH THE CUPOLA UP, IT FALLS OUT OF THE WAY.
	pshu a
...
@c	nop ; close (cupola.holder==top of shaft)
	...
	ldx #description_table
	lda #93 ; YOU'VE ALREADY DONE THAT.
	pshu a
	jsr print_table_entry
	jsr PRINTCR
@d	nop ; end else
...
@a	nop ; close (wrench.holder == player)
	...
	ldx #description_table
	lda #95 ; YOU DON'T HAVE ANYTHING TO DO THAT WITH.
	pshu a
	jsr print_table_entry
	jsr PRINTCR
@f	nop ; end else
